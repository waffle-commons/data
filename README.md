...
**Note:** Replace `YOUR_CODECOV_TOKEN_HERE` in the Codecov badge URL if you integrate Codecov. Also, replace `Data` placeholders in badges after running the configuration script or manually.
...
Execute the provided configuration script, passing the PascalCase component name as the first and only argument. This script will automatically replace the placeholder Data in file contents, filenames, and directory names.
```shell
