...
enum SQLDialect
{
     ...
     * which the SQR caller is responsible for supplying.
     */
    public function paginate(?int $limit, ?int $offset): string
    {
        ...
        return match ($this) {
            self::MSSQL => $this->paginateOffsetFetch($limit, $offset),
            self::MySQL, self::SQLite => $this->paginateLimitOffset($limit, $offset),
        };
    }
    ...
    {
        return match ($this) {
            self::MySQL => '`' . str_replace('`', '``', $segment) . '`',
            self::SQLite => '"' . str_replace('"', '""', $segment) . '"',
    ...
    {
        // MySQL/SQLite require a LIMIT before an OFFSET; when only an offset is
        // given, use the dialect's "no upper bound" sentinel.
        $sentinel = $this === self::MySQL ? '18446744073709551615' : '-1';
